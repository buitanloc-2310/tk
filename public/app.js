        ...
        <div class="card-wallet">
          ...
          ${
            d.items?.length
              ?d.items.map(
                  x=>`
                    <div class="member-card ${cardClass(x)}">
                      ...
                      <h3>
                        ${esc(x.card_type_name||'THẺ THÀNH VIÊN')}
                      </h3>

                      <div>
                        ${esc(state.me.person.full_name)}
                      </div>
              ...
              :'<div class="empty">Chưa có thẻ điện tử.</div>'
          }
...
if(tab==='membership'){
  ...
  if(tab==='card'){
    ...
    $$('[data-print-card]').forEach(btn=>btn.onclick=()=>{
      const x=d.cards.find(v=>v.id===btn.dataset.printCard); if(!x)return;
      const w=window.open('','_blank','width=900,height=650');
      const verifyUrl=location.origin+'/verify?code='+encodeURIComponent(x.verify_token);
      w.document.write(`<!doctype html><html><head><title>${esc(x.card_number)}</title><style>body{font-family:Arial,sans-serif;padding:30px}.sheet{width:86mm;min-height:54mm;border:1px solid #bbb;border-radius:12px;padding:8mm;box-sizing:border-box}.photo{width:24mm;height:30mm;object-fit:cover;border-radius:3mm}.row{display:flex;gap:6mm}.small{font-size:10px;color:#555}h2{font-size:16px;margin:3px 0}.qr{margin-top:8px;font-size:9px;word-break:break-all}@media print{body{padding:0}.sheet{border:0}}</style></head><body><div class="sheet"><div class="row"><img class="photo" src="${esc(p.avatar_url||'/sfn-logo.png')}"><div><b>SKY FIRST NETWORK</b><h2>${esc(x.card_type_name)}</h2><b>${esc(p.full_name)}</b><div>${esc(p.member_code||'')}</div><div>${esc(x.card_number)}</div><div>${esc(x.title_on_card||'')}</div><div class="small">${esc(x.org_name||'SFN')}</div><div class="small">${esc(x.issued_at||'')} → ${esc(x.expires_at||'Không thời hạn')}</div><div class="small">${esc(x.status)}</div></div></div><div class="qr">Xác minh: ${esc(verifyUrl)}</div></div><script>setTimeout(()=>window.print(),500)<\/script></body></html>`);w.document.close();
    });
