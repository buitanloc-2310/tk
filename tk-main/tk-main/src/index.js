...
export default{
  async fetch(request,env){
    ...
    // Member Portal deploys are updated frequently. Do not let an old login/app shell
    // survive a new Worker deployment in browser/CDN caches. Fingerprinted media can
    // still be cached by the Assets platform, while entry documents/scripts revalidate.
    if (url.pathname === '/' || url.pathname.endsWith('.html') || url.pathname === '/app.js' || url.pathname === '/styles.css') {
      ...
      assetHeaders.set('x-sfn-member-build', '20260920-identity-rebuild');
    }
