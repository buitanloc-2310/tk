...
async function api(req,env,url){
 ...
 if(url.pathname==='/api/me'&&req.method==='PATCH'){
  ...
  if(!next.full_name)return json({error:'FULL_NAME_REQUIRED'},400);
 ...
 if(url.pathname==='/api/admin/members'&&req.method==='GET'){
  ...
  const page=Math.max(1,Number(url.searchParams.get('page')||1)),limit=Math.min(100,Math.max(10,Number(url.searchParams.get('limit')||50))),q=clean(url.searchParams.get('q'),100),status=clean(url.searchParams.get('status'),30),w=[],b=[];
 ...
 if(url.pathname==='/api/admin/members'&&req.method==='POST'){
  ...
  const b=await bodyJson(req),pw=String(b.password||'');if(!clean(b.full_name,160)||!clean(b.username,80)||pw.length<10)return json({error:'INVALID_DATA'},400);
 ...
 if(subMatch&&req.method==='POST'){
  ...
  if(op==='certificate'){
   ...
   await env.DB.batch([
    ...
    env.DB.prepare(`INSERT INTO notifications(id,person_id,org_node_id,type,title,body) VALUES(?,?,?,'certificate','Bạn có chứng nhận mới',?)`).bind(uid('notice'),pid,b.org_node_id||null,`Chứng nhận "${clean(b.title,240)}" đã được cấp trên SFN Member Portal.`)
   ]);
