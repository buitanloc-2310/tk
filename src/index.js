...
async function api(req,env,url){
 ...
 if(url.pathname==='/api/setup'&&req.method==='POST'){
  ...
  await env.DB.batch([
   env.DB.prepare(`INSERT INTO people(id,member_code,full_name,email,joined_at,status,is_member) VALUES(?,?,?,?,DATE('now'),'active',0)`).bind(pid,'SYS-ADMIN-0001',name,email||null),
 ...
 if(url.pathname==='/api/public/verify'&&req.method==='GET'){
  ...
  if(card)return json({type:'card',valid:card.status==='active',record:card});
 ...
 if(url.pathname==='/api/me'&&req.method==='GET'){
  ...
  return json({person,memberships:memberships.results||[],cards:cards.results||[],permissions:(perms.results||[]).map(x=>x.code),scopes:scopes.results||[],is_super:await isSuper(env,s.account_id),force_password_change:!!s.force_password_change});
 }
 ...
 if(url.pathname==='/api/admin/org'&&req.method==='POST'){if(!(await hasPerm(env,s.account_id,'org.manage')))return json({error:'FORBIDDEN'},403);const b=await bodyJson(req),id=uid('org');if(!clean(b.code,80)||!clean(b.name,200))return json({error:'INVALID_DATA'},400);await env.DB.prepare(`INSERT INTO org_nodes(id,parent_id,code,name,short_name,node_type,status,sort_order) VALUES(?,?,?,?,?,?,?,?)`).bind(id,b.parent_id||'org_sfn',clean(b.code,80),clean(b.name,200),clean(b.short_name,80)||null,clean(b.node_type,80)||'unit','active',Number(b.sort_order||0)).run();await audit(env,s.account_id,'org_created','org_node',id,b.parent_id||'org_sfn',{code:b.code,name:b.name});return json({ok:true,id})}
 ...
 if(url.pathname==='/api/admin/members'&&req.method==='GET'){
  ...
  const page=Math.max(1,Number(url.searchParams.get('page')||1)),limit=Math.min(100,Math.max(10,Number(url.searchParams.get('limit')||50))),q=clean(url.searchParams.get('q'),100),status=clean(url.searchParams.get('status'),30),w=['p.is_member=1'],b=[];
 ...
 if(url.pathname==='/api/admin/members'&&req.method==='POST'){
  ...
  await env.DB.batch([
   ...
   env.DB.prepare(`INSERT INTO accounts(id,person_id,username,email,password_hash,password_salt,password_iterations,force_password_change) VALUES(?,?,?,?,?,?,?,1)`).bind(aid,pid,clean(b.username,80).toLowerCase(),clean(b.email,200).toLowerCase()||null,hash,salt,it),
 ...
 if(detailMatch&&req.method==='GET'){
  if(!(await hasPerm(env,s.account_id,'member.view')))return json({error:'FORBIDDEN'},403);
  const pid=decodeURIComponent(detailMatch[1]),person=await env.DB.prepare(`SELECT p.*,a.id account_id,a.username,a.email account_email,a.is_locked,a.force_password_change,a.last_login_at FROM people p LEFT JOIN accounts a ON a.person_id=p.id WHERE p.id=?`).bind(pid).first();
  if(!person)return json({error:'NOT_FOUND'},404);
  const [memberships,certificates,achievements,cards,documents,scopes]=await Promise.all([
  ...
  return json({person,memberships:memberships.results||[],certificates:certificates.results||[],achievements:achievements.results||[],cards:cards.results||[],documents:documents.results||[],scopes:scopes.results||[]});
 }
 if(detailMatch&&req.method==='PATCH'){
  if(!(await hasPerm(env,s.account_id,'member.edit')))return json({error:'FORBIDDEN'},403);
  const pid=decodeURIComponent(detailMatch[1]),b=await bodyJson(req),p=await env.DB.prepare('SELECT * FROM people WHERE id=?').bind(pid).first();if(!p)return json({error:'NOT_FOUND'},404);
 ...
 if(certFileMatch&&req.method==='POST'){
  if(!(await hasPerm(env,s.account_id,'certificate.manage')))return json({error:'FORBIDDEN'},403);const pid=decodeURIComponent(certFileMatch[1]);if(!(await env.DB.prepare('SELECT 1 FROM people WHERE id=?').bind(pid).first()))return json({error:'NOT_FOUND'},404);
 ...
 const subMatch=url.pathname.match(/^\/api\/admin\/members\/([^/]+)\/(certificate|achievement|membership|card|document|reset-password|lock|scope)$/);
 if(subMatch&&req.method==='POST'){
  const pid=decodeURIComponent(subMatch[1]),op=subMatch[2],b=await bodyJson(req),person=await env.DB.prepare(`SELECT p.*,a.id account_id FROM people p LEFT JOIN accounts a ON a.person_id=p.id WHERE p.id=?`).bind(pid).first();
  if(!person)return json({error:'NOT_FOUND'},404);
  if(op==='certificate'){
  ...
  if(op==='membership'){
   if(!(await hasPerm(env,s.account_id,'member.edit')))return json({error:'FORBIDDEN'},403);
   if(!b.org_node_id)return json({error:'ORG_REQUIRED'},400);const id=uid('membership');
 ...
 if(reqReview&&req.method==='POST'){
  ...
  const cnt=await env.DB.prepare(`SELECT COUNT(*) c FROM people WHERE is_member=1`).first(),pid=uid('person'),aid=uid('account'),code=memberCode(Number(cnt?.c||0)+1),salt=token(),it=100000,hash=await pbkdf2(pw,salt,it);
  await env.DB.batch([
   env.DB.prepare(`INSERT INTO people(id,member_code,full_name,display_name,date_of_birth,gender,nationality,id_number,id_issue_date,id_issue_place,email,phone,permanent_address,temporary_address,avatar_url,joined_at,status,is_member) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,DATE('now'),'active',1)`).bind(pid,code,r.full_name,r.display_name,r.date_of_birth,r.gender,r.nationality,r.id_number,r.id_issue_date,r.id_issue_place,r.email,r.phone,r.permanent_address,r.temporary_address,r.avatar_url),
 ...
 if(url.pathname==='/api/admin/meta'&&req.method==='GET'){
  if(!(await hasPerm(env,s.account_id,'member.view')))return json({error:'FORBIDDEN'},403);
  const [orgs,cards,roles]=await Promise.all([env.DB.prepare(`SELECT id,code,name,short_name,node_type,parent_id,status FROM org_nodes ORDER BY sort_order,name`).all(),env.DB.prepare(`SELECT id,code,name FROM card_types WHERE active=1 ORDER BY name`).all(),env.DB.prepare(`SELECT id,code,name FROM roles ORDER BY name`).all()]);
  return json({orgs:orgs.results||[],card_types:cards.results||[],roles:roles.results||[]});
 }
