...
async function api(req,env,url){
  ...
  if(url.pathname==='/api/public/account-request'&&req.method==='POST'){
    ...
    const email=clean(b.email,200).toLowerCase();
    const username=
      clean(b.desired_username,80).toLowerCase();
    const idno=clean(b.id_number,80);
    ...
    if(await env.DB.prepare(`
      ...
      WHERE status='pending'
        AND (
          lower(email)=?
          OR id_number=?
          OR lower(desired_username)=?
        )
      LIMIT 1
    `).bind(email,idno,username).first()){
      return json({error:'REQUEST_ALREADY_PENDING'},409);
    }

    const id=uid('request');

    const code=
      `SFN-REQ-${String(Date.now()).slice(-8)}-`+
      crypto.randomUUID()
        .slice(0,4)
        .toUpperCase();
    ...
    return json({
      ...
      message:
        ...
        'SFN dự kiến kiểm tra trong 12–24 giờ; '+
        'có thể sớm hoặc trễ hơn tùy lưu lượng và '+
        'khối lượng công việc. Vui lòng thường xuyên '+
        'kiểm tra email và lưu mã yêu cầu để tra cứu trạng thái.'
    });
  }
  ...
  if(
    url.pathname==='/api/admin/account-requests'&&
    req.method==='GET'
  ){
    if(!(await hasPerm(
      env,
  ...
  const reqReview=
    url.pathname.match(
      /^\/api\/admin\/account-requests\/([^/]+)\/(approve|reject)$/
    );
  ...
  if(reqReview&&req.method==='POST'){
    ...
    const nextNo=
      last?.member_code
        ?Number(
            last.member_code.slice(4)
          )+1
        :1;

    const pid=uid('person');
    const aid=uid('account');
    const code=memberCode(nextNo);
    const salt=token();
    const it=100000;
    const hash=await pbkdf2(
      pw,
    ...
    return json({
      ...
      note:
        ...
        'Gửi thông tin đăng nhập cho người yêu cầu '+
        'qua kênh email chính thức.'
    });
  }

  // =========================================================
  // REVIEW EXTERNAL CERTIFICATE
  // =========================================================

  const certReview=
    url.pathname.match(
      /^\/api\/admin\/certificates\/([^/]+)\/review$/
    );
  ...
  if(certReview&&req.method==='POST'){
    ...
    if(cert.source_type!=='external'){
      return json({
        error:'INTERNAL_CERTIFICATE'
      },409);
    }
    ...
    `).bind(
      uid('notification'),
      cert.person_id,
      cert.org_node_id||null,
      status==='verified'
        ?'Chứng nhận bên ngoài đã được xác minh'
        :'Chứng nhận bên ngoài chưa được xác minh',
      status==='verified'
        ?'Chứng nhận bạn cung cấp đã được SFN xác minh.'
        :'Chứng nhận bạn cung cấp đã bị từ chối xác minh.'
    ).run();

    await audit(
      env,
      s.account_id,
      'external_certificate_reviewed',
      'certificate',
      cid,
      cert.org_node_id||null,
      {
